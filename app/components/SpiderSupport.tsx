...
export default function SpiderSupport(){
 ...
 function send(text=q){const v=text.trim();if(!v)return;setMessages(m=>[...m,"You: "+v,"Spider Support: "+answer(v)]);setQ("");}
 return <div className={"supportWidget "+(open?"open":"")}><button className="supportLauncher" onClick={()=>setOpen(!open)}>{open?"×":"SPIDER SUPPORT"}</button>{open&&<div className="supportPanel"><div className="supportHead"><div><strong>Spider Support Support</strong><small>Ask about the studio, team, games, hiring or acquisitions.</small></div></div><div className="supportMessages">{messages.length===0&&<div className="supportWelcome">Hi — I’m Spider Studios Support. What can I help you find?<div className="supportSuggestions">{suggestions.map(s=><button key={s} onClick={()=>send(s)}>{s}</button>)}</div></div>}{messages.map((m,i)=><div className={m.startsWith("You:")?"supportMsg user":"supportMsg"} key={i}>{m.replace(/^(You:|Spider Support:) /,"")}</div>)}</div><div className="supportInput"><input value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Ask Spider Support..." /><button onClick={()=>send()}>SEND</button></div></div>}</div>
}